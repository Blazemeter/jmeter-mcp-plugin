...
/**
 * Swing GUI for the {@link McpClientConfig} element. Renders one panel per transport type and swaps
 * them with a {@link CardLayout} when the user changes the {@code Transport} selection.
 */
public class McpClientConfigGui extends AbstractConfigGui implements Scrollable {
  ...
  private final JComboBox<TransportType> transportCombo = new JComboBox<>(TransportType.values());
  ...
  private JPanel buildServerLaunchPanel() {
    serverLaunchPanel.setBorder(
        BorderFactory.createTitledBorder("Server launch (Start Now — HTTP/SSE only)"));
    GridBagConstraints c = GridBagForm.horizontalRowConstraints();
  ...
  public void modifyTestElement(TestElement element) {
    ...
    TransportType selected = (TransportType) transportCombo.getSelectedItem();
    cfg.setProperty(
        McpClientConfig.TRANSPORT, selected != null ? selected.name() : TransportType.STDIO.name());
    ...
    cfg.setProperty(
        McpClientConfig.KEEP_SERVER_RUNNING_AFTER_TEST,
    ...
    cfg.setProperty(
        McpClientConfig.SERVER_READY_PORT,
        GridBagForm.parseLong(serverReadyPortField.getText(), 3001L));
    cfg.setProperty(
        McpClientConfig.SERVER_STARTUP_WAIT_MS,
  ...
  public void configure(TestElement element) {
    ...
    transportCombo.setSelectedItem(
        TransportType.fromString(
            cfg.getPropertyAsString(McpClientConfig.TRANSPORT, TransportType.STDIO.name())));
    ...
    clientNameField.setText(
        cfg.getPropertyAsString(McpClientConfig.CLIENT_NAME, "jmeter-mcp-plugin"));
    clientVersionField.setText(cfg.getPropertyAsString(McpClientConfig.CLIENT_VERSION, "0.1.0"));
    requestTimeoutField.setText(
        String.valueOf(cfg.getPropertyAsLong(McpClientConfig.REQUEST_TIMEOUT_MS, 30_000L)));
    initTimeoutField.setText(
        String.valueOf(cfg.getPropertyAsLong(McpClientConfig.INIT_TIMEOUT_MS, 30_000L)));
    ...
    serverReadyPortField.setText(
        String.valueOf(cfg.getPropertyAsLong(McpClientConfig.SERVER_READY_PORT, 3001L)));
    serverStartupWaitField.setText(
        String.valueOf(cfg.getPropertyAsLong(McpClientConfig.SERVER_STARTUP_WAIT_MS, 60_000L)));
    showSelectedTransport();
    refreshConnectionStatus();
  }
