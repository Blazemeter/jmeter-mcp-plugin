...
/**
 * Process-wide registry of live {@link McpSyncClient} instances keyed by the configuration element
 * name.
 *
 * <p>JMeter clones config elements per thread, but the underlying connection is heavy and must be
 * shared. {@link #registerDeferred(String, McpClientSettings)} stores settings during {@code
 * testStarted()} when lazy connect is configured. {@link #connectOnStartup(String,
 * McpClientSettings)} registers settings and schedules connect on a background thread so {@code
 * testStarted()} returns immediately (same responsiveness as lazy init). The first sampler that
 * calls {@link #getOrConnect(String)} waits for that background connect or performs lazy connect if
 * startup connect was not requested.
 *
 * <p>All use of a connected client (including RPCs from samplers) must go through {@link
 * #withClient(String, Function)} so concurrent threads do not call the MCP SDK in parallel on the
 * same transport (STDIO uses a unicast outbound sink that rejects concurrent {@code tryEmitNext}
 * with "Failed to enqueue message").
 */
public final class McpClientRegistry {
  ...
  /**
   * Store settings for a named client. Does not open a connection — that happens on the first
   * {@link #getOrConnect(String)} call.
   */
  ...
  /**
   * Connect immediately from the GUI (Start Now). Replaces any existing client registered under the
   * same name.
   */
  ...
  /**
   * Disconnect a GUI-started client. Returns whether a managed server subprocess started via Start
  ...
  /**
   * When Start Now (or a prior test) left a client connected, keep it and only refresh settings for
   * the upcoming run.
   */
  private boolean adoptExistingConnection(String name, McpClientSettings settings) {
    ...
    LOG.info(
        "Reusing existing MCP client '{}' for test run (transport {})",
        name,
  ...
  /**
   * Register settings and schedule connect on a background thread. Does not block {@code
   * testStarted()}.
   */
  public void connectOnStartup(String name, McpClientSettings settings) {
    ...
    if (previous != null) {
      LOG.warn(
          "MCP client '{}' settings replaced (was transport {}, now {})",
          name,
          previous.getTransport(),
          settings.getTransport());
    }
  ...
  /**
   * Returns an initialized client, creating it on first use from deferred settings registered via
   * {@link #registerDeferred(String, McpClientSettings)}.
   *
   * <p>Prefer {@link #withClient(String, Function)} for sampler and GUI catalog calls so RPCs are
   * serialized per client name.
   */
  ...
  /**
   * Resolves (and if needed connects) the named client, then runs {@code action} while holding the
   * per-client lock so only one thread uses the transport at a time.
   */
  ...
  /**
   * @param connectMillisHolder if non-null and length &gt; 0, receives time spent in {@link
   *     #resolveClient(String)} (connect / wait for startup connect)
   */
  public <T> T withClient(
      String name, Function<McpSyncClient, T> action, long[] connectMillisHolder) {
  ...
  /**
   * Runs {@code action} under the per-client lock without connecting. For GUI catalog sync when the
   * client was started via Start Now.
   */
  ...
  private McpSyncClient resolveClient(String name) {
    ...
    if (pending != null) {
      try {
        ...
        LOG.warn(
            "Startup connect failed for MCP client '{}': {} — retrying", name, cause.getMessage());
  ...
  private McpSyncClient connectClient(String name, String reason) {
    ...
    LOG.info(
        "Connecting MCP client '{}' ({}, transport {})", name, reason, settings.getTransport());
  ...
  private static void closeQuietly(McpSyncClient client) {
    try {
      ...
      if (isExpectedShutdownFailure(ex)) {
        LOG.debug(
            "MCP client graceful close failed during shutdown (server may be gone): {}",
            ex.getMessage());
      } else {
