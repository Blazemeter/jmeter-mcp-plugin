 ...
 * factory hides the transport-specific wiring so the JMeter components only need to know about
 * settings.
 */
public final class McpClientFactory {
  ...
  private static String requireUrl(McpClientSettings s, String label) {
    ...
    if (url == null || url.isBlank()) {
      throw new IllegalArgumentException("MCP " + label + " transport requires a server URL");
    }
    return url;
  }
