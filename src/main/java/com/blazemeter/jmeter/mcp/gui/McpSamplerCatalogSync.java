...
/**
 * GUI helper that lists tools, resources, or prompts from a connected preview client and fills
 * {@link EditableCatalogField} choices.
 */
public final class McpSamplerCatalogSync {
  ...
  public static void syncAsync(
      String configName, McpOperation operation, EditableCatalogField field, Runnable onComplete) {
    field.getSyncButton().setEnabled(false);
    new SwingWorker<List<String>, Void>() {
      @Override
      protected List<String> doInBackground() {
        return McpClientRegistry.getInstance()
            .withConnectedClient(
                configName,
                client -> {
                  if (client == null) {
                    throw new IllegalStateException(
                        "No connected MCP client for '"
                            + configName
                            + "'. "
      ...
      protected void done() {
        ...
        try {
          ...
          JOptionPane.showMessageDialog(
              field, cause.getMessage(), "MCP catalog sync failed", JOptionPane.ERROR_MESSAGE);
        }
      }
    }.execute();
  }
