...
/**
 * Swing GUI for {@link McpSampler}. Shows only the input fields relevant to the currently selected
 * {@link McpOperation}.
 */
public class McpSamplerGui extends AbstractSamplerGui implements Scrollable {
  ...
  private final JComboBox<McpOperation> operationCombo = new JComboBox<>(McpOperation.values());

  private final EditableCatalogField toolNameField = new EditableCatalogField(25, "Sync");
  private final EditableCatalogField resourceUriField = new EditableCatalogField(35, "Sync");
  private final EditableCatalogField promptNameField = new EditableCatalogField(25, "Sync");
  ...
  private void init() {
    ...
    toolNameField
        .getSyncButton()
        .addActionListener(e -> syncCatalog(McpOperation.CALL_TOOL, toolNameField));
    resourceUriField
        .getSyncButton()
        .addActionListener(e -> syncCatalog(McpOperation.READ_RESOURCE, resourceUriField));
    promptNameField
        .getSyncButton()
        .addActionListener(e -> syncCatalog(McpOperation.GET_PROMPT, promptNameField));
    updateCard();
  ...
  public void modifyTestElement(TestElement element) {
    ...
    McpOperation op = (McpOperation) operationCombo.getSelectedItem();
    sampler.setProperty(McpSampler.OPERATION, op != null ? op.name() : McpOperation.PING.name());
  ...
  public void configure(TestElement element) {
    ...
    operationCombo.setSelectedItem(
        McpOperation.fromString(
