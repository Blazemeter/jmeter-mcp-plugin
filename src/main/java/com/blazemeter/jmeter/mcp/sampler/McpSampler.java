 ...
 *
 * <p>The sampler resolves the {@link McpSyncClient} via {@link McpClientRegistry} using the
 * configured {@link #CONFIG_NAME} (which must match the {@code Variable Name} of an {@code MCP
...
public class McpSampler extends AbstractSampler {
  ...
  public SampleResult sample(Entry e) {
    ...
    McpOperation operation =
        McpOperation.fromString(getPropertyAsString(OPERATION, McpOperation.PING.name()));
    ...
    try {
      SampleResult sample =
          McpClientRegistry.getInstance()
              .withClient(
                  configName,
                  client -> {
                    if (client == null) {
                      throw new IllegalStateException(
                          "No MCP client settings for '"
                              + configName
  ...
  /**
   * Builds HTTP-style response headers with MCP session metadata so View Results Tree shows
   * handshake data separately from the operation body.
   */
  ...
  /**
   * Session / initialize handshake (protocol, capabilities, server info, instructions) — same
   * content the SDK logs at INFO from {@code LifecycleInitializer}.
   */
    ...
    switch (op) {
      case PING: {
        ...
        if (pingResult == null) {
          ...
          body.put(
              "note", "McpSyncClient.ping() returned null; the ping RPC completed successfully.");
          return body;
        }
        return pingResult;
      }
  ...
  protected Map<String, Object> parseArguments() {
    ...
    try {
      ...
      throw new IllegalArgumentException("Arguments must be a JSON object: " + ex.getMessage(), ex);
    }
  }
  ...
  private static String toJson(Object value) {
    ...
    try {
      ...
      LOG.debug(
          "Falling back to toString() for {}: {}", value.getClass().getName(), ex.getMessage());
  ...
  private static String toPrettyJson(Object value) {
    ...
    try {
      ...
      LOG.debug(
          "Falling back to compact JSON for {}: {}", value.getClass().getName(), ex.getMessage());
      return toJson(value);
    }
  }
